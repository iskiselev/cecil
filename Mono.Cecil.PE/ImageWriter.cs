...
namespace Mono.Cecil.PE {
	...
	sealed class ImageWriter : BinaryStreamWriter {
		...
		void GetWin32Resources ()
		{
			...
			DataDirectory l_win32_resources_directory = module.Image.Win32Resources;
			var size = l_win32_resources_directory.Size;

			if (size > 0) {
				win32_resources = module.Image.GetReaderAt (l_win32_resources_directory.VirtualAddress, size, (s, reader) => new ByteBuffer (reader.ReadBytes ((int) s)));
			}
		}
		...
		void BuildSections ()
		{
			bool hasResources = win32_resources != null ||
			                    (win32_resources_directory != null && win32_resources_directory.Entries.Count > 0);
			if (hasResources)
				sections++;
